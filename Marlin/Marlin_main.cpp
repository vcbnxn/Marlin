...
//  ====== DGlass3D Extruder code start ======
      ...
      if(degHotend(active_extruder) > EXTRUDE_MINTEMP){        
        ...
        if(NUM_SERVOS >= 0){          
          ...
          int desired_servo_angle = DEFAULT_SERVO_ANGLE[(int)active_extruder];
          //SERIAL_PROTOCOLPGM("Extruder: ");
          //SERIAL_PROTOCOL((int)active_extruder);
          //SERIAL_PROTOCOLPGM(" Current:");  
          //SERIAL_PROTOCOL(current_servo_angle);
          //SERIAL_PROTOCOLPGM(" Desired:")
          //SERIAL_PROTOCOL(desired_servo_angle);             
          concurrentMove(current_servo_angle, desired_servo_angle, (int)active_extruder);
...
void concurrentMove(int current_servo_angle, int desired_servo_angle, int active_extruder)
{
  ...
  if (current_servo_angle <= desired_servo_angle){
    for (int active_servo_position = current_servo_angle; active_servo_position <= desired_servo_angle; active_servo_position++){
      ...
      destination[E_AXIS] = destination[E_AXIS] + EXTRUSION_LENGTH;
      plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], destination[E_AXIS], 1.5, active_extruder);                 
      delay(MOVEMENT_ITERATION_DELAY);
    }
  }
  else{
    for (int active_servo_position = current_servo_angle; active_servo_position >= desired_servo_angle; active_servo_position--){
